...
public class AtaService {
	...
	public List<Ata> getByProfessor(Professor professor) {
		...
		for(Ata ata : this.getAll()){
			for(Tempo tempo : ata.getTempos()){
				if(tempo.getDisciplina().getProfessor().equals(professor)){
					atas.add(ata);
					break;
				}
			}
	...
	public List<Ata> getByXerife(Aluno xerife) {
		...
		// ResponseEntity<Ata[]> response =
		// restTemplate.getForEntity(ServiceName.ata + "/xerife/{id}",
		// Ata[].class, id);
		return atas;
	}
	...
	public List<Ata> getByCoordenador(Professor coordenador) {
		...
		// ResponseEntity<Ata[]> response =
		// restTemplate.getForEntity(ServiceName.ata + "/coordenador/{id}",
		// Ata[].class, coordenador.getId());
		return atas;
	}
